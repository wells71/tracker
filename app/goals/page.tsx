...
// Server data fetched in a wrapper
...
function GoalsClient({ goals: initial }: { goals: Goal[] }) {
    ...
    <div>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="font-mono text-[11.5px] text-muted-foreground">{initial.length} active goals</p>
        </div>
        <GoalModal>
          <Button size="sm">
      ...
      <GoalModal goal={editGoal} open={editOpen} onOpenChange={o => { setEditOpen(o); if (!o) setEditGoal(null) }} />
      ...
      <Card>
        <CardContent className="divide-y divide-border p-0">
          {initial.length
            ? initial.map(g => (
                <div key={g.id} className="flex items-center gap-4 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="font-mono text-[11px] text-muted-foreground">{g.sub}</p>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="font-mono text-[13px] font-semibold">{g.pct}%</p>
                    <div className="mt-1.5 flex gap-1 justify-end">
                      <button onClick={() => { setEditGoal(g); setEditOpen(true) }}
                      ...
                      <button onClick={() => handleDelete(g.id)} disabled={pending}
                        className="rounded border border-border p-1 text-muted-foreground transition-colors hover:border-red-500/40 hover:text-red-400">
                        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-2.5 w-2.5">
            ...
            : <p className="py-12 text-center font-mono text-xs text-muted-foreground">No goals yet — add your first one.</p>
          }
        </CardContent>
      </Card>
    </div>
